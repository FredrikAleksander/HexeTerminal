...
namespace
{
    ...
    struct EmojiFont
    {
        ...
        operator bool() const { return Face != nullptr; }
    };

    struct EmojiData
    {
        ImVector<std::pair<ImWchar, int>> m_emojiRects;
    };
    ...
    struct FreeTypeFont
    {
        ...
        ImVector<std::pair<ImWchar, int>> Emojis;
    };
...
#define FT_CEIL(X) (((X + 63) & -64) / 64)
    ...
    bool FreeTypeFont::InitFont(FT_Library ft_library, const ImFontConfig &cfg, unsigned int extra_user_flags)
    {
        ...
        if (UserFlags & ImGuiFreeTypeEx::NoHinting)
            LoadFlags |= FT_LOAD_NO_HINTING;
        if (UserFlags & ImGuiFreeTypeEx::NoAutoHint)
            LoadFlags |= FT_LOAD_NO_AUTOHINT;
        if (UserFlags & ImGuiFreeTypeEx::ForceAutoHint)
            LoadFlags |= FT_LOAD_FORCE_AUTOHINT;
        if (UserFlags & ImGuiFreeTypeEx::LightHinting)
            LoadFlags |= FT_LOAD_TARGET_LIGHT;
        else if (UserFlags & ImGuiFreeTypeEx::MonoHinting)
            LoadFlags |= FT_LOAD_TARGET_MONO;
        else
            LoadFlags |= FT_LOAD_TARGET_NORMAL;

        if (UserFlags & ImGuiFreeTypeEx::Monochrome)
    ...
    const FT_Glyph_Metrics *FreeTypeFont::LoadGlyph(uint32_t codepoint)
    {
        ...
        if (UserFlags & ImGuiFreeTypeEx::Bold)
            FT_GlyphSlot_Embolden(slot);
        if (UserFlags & ImGuiFreeTypeEx::Oblique)
        {
    ...
    bool AddEmoji(ImFontAtlas *atlas, ImFont *dstFont, ImVector<std::pair<ImWchar, int>> &rects, float charWidth, float charHeight, EmojiFont &font)
    {
        ...
        for (auto codepoint : font.glyphCodepoints)
        {
            ...
            if (glyph_index == 0)
                continue;
            int rect = atlas->AddCustomRectFontGlyph(dstFont, codepoint, emojiWidth, emojiHeight, charWidth, emojiOffset);

            rects.push_back(std::make_pair(rect, codepoint));
        }
        return true;
    }
    ...
    void BlitEmoji(FT_GlyphSlot glyph, int w, int h, unsigned char *pixels)
    {
        ...
        srcRect.x = 0;
        srcRect.y = 0;
        srcRect.w = glyph->bitmap.width;
        srcRect.h = glyph->bitmap.rows;
        ...
        SDL_BlitScaled(sourceSurface, &srcRect, destSurface, &dstRect);
    ...
    bool BuildEmoji(ImFontAtlas *atlas, ImFont *font, const ImVector<std::pair<ImWchar, int>> rects, float charWidth, float charHeight, EmojiFont &emojiFont)
    {
        ...
        else
        {
            ...
            req.type = FT_SIZE_REQUEST_TYPE_REAL_DIM;
            req.width = 0;
            req.height = (uint32_t)charHeight * 64;
            req.horiResolution = 0;
            req.vertResolution = 0;
            FT_Request_Size(emojiFont.Face, &req);

            // // Update font info
            // FT_Size_Metrics metrics = Face->size->metrics;
            // Info.PixelHeight = (uint32_t)charHeight;
            // Info.Ascender = (float)FT_CEIL(metrics.ascender);
            // Info.Descender = (float)FT_CEIL(metrics.descender);
            // Info.LineSpacing = (float)FT_CEIL(metrics.height);
            // Info.LineGap = (float)FT_CEIL(metrics.height - metrics.ascender + metrics.descender);
...
bool ImFontAtlasBuildWithFreeType(FT_Library ft_library, ImFontAtlas *atlas, unsigned int extra_flags, const ImVector<unsigned char> &emoji_font_data)
{
    ...
    EmojiFont emojiFont(ft_library, emoji_font_data);
...
bool ImGuiFreeTypeEx::BuildFontAtlas(ImFontAtlas *atlas, unsigned int extra_flags, const ImVector<unsigned char> &color_emoji_font)
{
    ...
    bool ret = ImFontAtlasBuildWithFreeType(ft_library, atlas, extra_flags, color_emoji_font);
    FT_Done_Library(ft_library);

    return ret;
}

void ImGuiFreeTypeEx::SetAllocatorFunctions(void *(*alloc_func)(size_t sz, void *user_data), void (*free_func)(void *ptr, void *user_data), void *user_data)
{
